...
// until you reach a leaf. When you reach a leaf node, back up to its parent, check for a right child and
//  visit it if there is one. If there is not a child, you've explored its left and right subtrees fully.
// If there is a right child, traverse its left subtree then its right in the same manner. Keep doing this
...
// Given a tree and an integer, k, in one operation, we need to swap the subtrees of all the nodes at each
// depth h, where h ∈ [k, 2k, 3k,...]. In other words, if h is a multiple of k, swap the left and right
// subtrees of that level.
// You are given a tree of n nodes where nodes are indexed from [1..n] and it is rooted at 1. You have to
// perform t swap operations on it, and after each swap operation print the in-order traversal of the current
// state of the tree.
// Function Description
// Complete the swapNodes function in the editor below. It should return a two-dimensional array where each
// element is an array of integers representing the node indices of an in-order traversal after a swap operation.
// swapNodes has the following parameter(s):
// - indexes: an array of integers representing index values of each , beginning with , the first element,
// as the root.
// - queries: an array of integers, each representing a  value.
// Input Format
// The first line contains n, number of nodes in the tree.
// Each of the next n lines contains two integers, a b, where a is the index of left child, and b is the
// index of right child of ith node.
...
// 2 1 3
// Explanation 0
// As nodes 2 and 3 have no children, swapping will not have any effect on them. We only have to swap the
// child nodes of the root node.
// 1   [s]       [s]       [s]
// / \   ->    / \   ->    / \
// 2   3 [s]   3   2 [s]   2   3
// 1                  1
// In the first case, k = 1, so we swap the left child with the right child.
// In the second case, k = 1, so we swap the right child with the left child.
// Sample Input 1
// 5
// 2 3
// -1 4
...
// Explanation 2
// Here we perform swap operations at the nodes whose depth is either 2 or 4 for  and then at nodes whose
// depth is 4 for .
// 1                     1                          1
// / \                   / \                        / \
// 2   3    [s]          2   3                      2   3
